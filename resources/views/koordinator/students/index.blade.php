				...
				<div class="modal-body">
					<form method="POST" action="/koordinator/students"> {{ csrf_field() }}
						...
						<select name="period" class="form-control">
							@foreach($period as $v)
							@if(isset($_GET['id']))
							<option value="{{ $v->id }}" @if($_GET['id']==$v->id) selected="selected"
								@endif>{{ $v->year }} Semester {{ $v->semester }}</option>
							@else
							<option value="{{ $v->id }}" @if($last_period==$v->id) selected="selected"
								@endif>{{ $v->year }} Semester {{ $v->semester }}</option>
							@endif
							@endforeach
						</select>
						...
						List Student (Format = NIM/username Nama Lengkap) (Contoh format dibawah)<br>
						<textarea rows=10 class="form-control" name=students>
13318001 Student Pertama
13318002 Student Kedua</textarea><br>
						<div align="right"><input class="btn btn-primary btn-sm" type="submit" value="Save" /></div>
					</form>
					<p></p>
				</div>
			</div>
	...
	<div class="row">
		...
		<div class="col-md-12">
			...
			<div class="alert alert-info">
				...
				<b>TIPS:</b> Pada halaman ini, koordinator dapat mendaftarkan mhs untuk mata kuliah Tugas Akhir 1
				sekaligus membuatkan user untuk masing-masing mahasiswa<br>(password, username, dan email dibuat
				otomatis sama dengan no induk yang kemudian dapat diubah oleh masing-masing mahasiswa)
			</div>
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					<select onchange="changeStatus(this.value)" class="form-control">
						@foreach($period as $v)
						@if(isset($_GET['id']))
						<option value="{{ $v->id }}" @if($_GET['id']==$v->id) selected="selected" @endif>{{ $v->year }}
							Semester {{ $v->semester }}</option>
						@else
						<option value="{{ $v->id }}" @if($last_period==$v->id) selected="selected" @endif>{{ $v->year }}
							Semester {{ $v->semester }}</option>
						@endif
						@endforeach
					</select>
					...
					<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
						<ul class="nav navbar-nav">
							<li>List Mahasiswa yang sudah terdaftar</li>
						</ul>
						<ul class="nav navbar-nav navbar-right">
							<li>
								<button id="editBtn" class="btn btn-primary btn-sm" data-toggle="modal"
									data-target="#myModalBulk">Tambahkan Peserta
								</button>
							</li>
						</ul>
					</div>
